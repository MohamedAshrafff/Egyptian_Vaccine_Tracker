...
for i in range(len(labels2)):
   sideeffect1.append(data.loc[(data['Vaccine Type'] == labels2[i])&(data['Side Effect']=='Diarrhea')].count()[0])
   
plt.bar(labels2,sideeffect1,color=cm.get_cmap("Pastel2").colors)
plt.ylabel('Number of people who suffered from diarrhea')
plt.xlabel('Vaccine Type')
plt.title('Highest vaccine causing diarrhea')
plt.tight_layout()
plt.show()