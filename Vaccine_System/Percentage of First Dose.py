...
have_taken = data.loc[data['First Dose'] == 1].count()[0]
have_not_taken_yet = data.loc[data['First Dose'] == 0].count()[0]
...
plt.pie([have_taken,have_not_taken_yet],labels=labels,colors=colors,autopct='%0.2f%%',
        shadow=True,
        wedgeprops=
        { 'linewidth' : 1, 'edgecolor' : "black" },startangle=80 )
plt.axis('equal')
plt.tight_layout()
plt.title('Percentage of first dose',y=0.97)
plt.show()